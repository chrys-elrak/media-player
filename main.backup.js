...
let $win, $playlistWin, $filePaths = new Set(); // GLOBAL VARIABLES
global.$playlist = [];
global.$current = null;
...
ipcMain.on('open-playlist', async (e, arg) => {
  if (arg.open) {
    ...
    current.state = eFileState.PLAY;
    $playlistWin.webContents.send("files-loaded", {
      playlist: $playlist,
      current
    });
...
function filesLoaded(eventName, o) {
  ...
  setPlaylist(setFiles(o.files, o.merge).map(f => new File(f)));
  arg = {
...
function setPlaylist(value) {
  global.$playlist = value;
  global.$current = value[0];
  global.$current.state = eFileState.PLAY;
}
